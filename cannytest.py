...
ret, thresh = cv2.threshold(image_norm,0, 65535, cv2.THRESH_TOZERO)
cv2.namedWindow("thresh",cv2.WINDOW_AUTOSIZE)
cv2.imshow("thresh",image)

# image_3chan = np.stack([image]*3,axis=-1) #stack for bgr/rgb

image_norm_uint8 = cv2.convertScaleAbs(image_norm)
image_canny = cv2.Canny(image_norm_uint8,100,150)
# collated = np.hstack((image_uint8,image_canny))
